    ...
    accountId: {
      ...
      beforeDestroy: beforeValidateHook,
			...
			afterCreate: function(instance, options) {
				...
				// Order.updateImages(imageKeys, data.imageExtraData);
			},
        ...
        function checkValue(value, config) {
            ...
            if (!( self.config && self.config.ideas && self.config.ideas.extraDataMustBeDefined === false )) {
              errors.push(`Order.extraData is not configured in site.config`)
            }
          }
        }
      ...
      api: {
			...
			mapMarkers: {
				...
				where: sequelize.or(
					...
					sequelize.and(
						{status: 'CLOSED'},
						sequelize.literal(`DATEDIFF(NOW(), Order.updatedAt) <= 90`)
					)
				)
			},
			...
			selectRunning: {
				where: sequelize.or(
					...
					sequelize.and(
						{status: 'DENIED'},
						sequelize.literal(`DATEDIFF(NOW(), Order.updatedAt) <= 7`)
					)
				)
			},
...
// 					ranked.forEach(idea => {
// 						Order.ranking = Order.status == 'DENIED' ? -10000 : Order.yes - Order.no;
// 					});
...
// 				});
	...
	Order.getRunning = function( sort, extraScopes ) {
	...
	Order.prototype.getUserVote = function( user ) {
	...
	Order.prototype.isRunning = function() {
		return this.status === 'OPEN'     ||
			this.status === 'CLOSED'   ||
	...
	// standaard stemvan
	Order.prototype.addUserVote = function( user, opinion, ip, extended ) {
	...
	Order.prototype.setUserVote = function( user, opinion, ip ) {
		let self = this;
		if (config.votes && config.votes.maxChoices) {
			...
			throw new Error('Order.setUserVote: missing params');
		}

	}

	Order.prototype.setModBreak = function( user, modBreak ) {
		return this.update({
			modBreak       : modBreak,
	...
	Order.prototype.setMeetingId = function( meetingId ) {
	...
	Order.prototype.updateImages = function( imageKeys, extraData ) {
		var self = this;
