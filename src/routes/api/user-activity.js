...
router
  ...
  .get(function (req, res, next) {
    ...
    return db.Idea.scope(['includeSite'])
      .findAll({ where })
      .then(function (rows) {
  ...
  .get(function (req, res, next) {
    if (!req.activities.includes('arguments')) return next();
    let where = { userId: req.userIds };
    return db.Argument.scope(['withIdea'])
      .findAll({ where })
      .then(function (rows) {
      ...
      if (activityConfig[which]) {
        const formattedAsActivities =
          req.results[which] && Array.isArray(req.results[which])
            ? req.results[which].map(resource => {
                ...
                return {
                  ...
                  description: resource[config.descriptionKey]
                    ? resource[config.descriptionKey].replace(/<[^>]+>/g, '')
                    : '',
                  type: config.type,
                  idea: idea,
