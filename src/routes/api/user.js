...
const formatOAuthApiCredentials = (site, which = 'default') => {
  let siteOauthConfig =
    (site && site.config && site.config.oauth && site.config.oauth[which]) ||
    {};
  let authClientId =
...
router
  ...
  .put(function (req, res, next) {
    ...
    let authServerUrl =
      ...
      config.authorization['auth-server-url'];
    let authUpdateUrl =
      authServerUrl + '/api/admin/user/' + req.results.externalUserId;
    let authClientId =
    ...
    fetch(authUpdateUrl, options)
      ...
      .then((result) => {
        return db.User.scope(['includeSite']).findOne({
          where: { id: userId, siteId: req.params.siteId },
    ...
    if (userForAllSites.length <= 1) {
      ...
      let siteOauthConfig =
        (req.site &&
          req.site.config &&
          req.site.config.oauth &&
          req.site.config.oauth['default']) ||
        {};
      let authServerUrl =
        siteOauthConfig['auth-server-url'] ||
        config.authorization['auth-server-url'];
      let authUserDeleteUrl =
        authServerUrl +
        '/api/admin/user/' +
        req.results.externalUserId +
        '/delete';
      let authClientId =
        siteOauthConfig['auth-client-id'] ||
        config.authorization['auth-client-id'];
