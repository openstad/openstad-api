...
router
  ...
  .all(function (req, res, next) {
    ...
    let hasModeratorRights =
      ...
      req.user.role === 'moderator'; // TODO: er staat een functie bovenin deze file; waarom gerbuik je die niet?
...
router
  ...
  .post(function (req, res, next) {
    ...
    db.Idea.findAll({ where: { id: ids, siteId: req.site.id } }).then(found => {
  ...
  .post(function (req, res, next) {
    ...
    actions.map(action => {
      switch (action.action) {
        ...
        case 'update':
          promises.push(
            db.Vote.update(action.vote, { where: { id: action.vote.id } })
          );
          break;
        case 'delete':
